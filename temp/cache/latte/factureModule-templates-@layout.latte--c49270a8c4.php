<?php
// source: C:\xampp\htdocs\Zbysaweb\app\factureModule/templates/@layout.latte

use Latte\Runtime as LR;

final class Templatec49270a8c4 extends Latte\Runtime\Template
{
	public $blocks = [
...
?>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
      <a class="navbar-brand" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("Homepage:default")) ?>">Přehled</a>
      <ul class="navbar-nav mr-auto">
        <li class="nav-item <?php
			if (($this->global->fn->isLinkCurrent)('Homepage:generatePdf')) {
				?> active<?php
			}
?>">
          <a class="nav-link" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("Homepage:generatePdf")) ?>">Generovat PDF Fakturu</a>
        </li>
        <li class="nav-item <?php
			if (($this->global->fn->isLinkCurrent)('Homepage:customer')) {
				?> active <?php
			}
?>">
          <a class="nav-link" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("Homepage:customer")) ?>">odběratelé</a>
        </li>
        <li class="nav-item <?php
			if (($this->global->fn->isLinkCurrent)('Homepage:myAddress')) {
				?> active <?php
			}
?>">
          <a class="nav-link" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("Homepage:myAddress")) ?>">moje údaje</a>
        </li>
      </ul>
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link" href="<?php echo LR\Filters::escapeHtmlAttr($this->global->uiControl->link("Out!")) ?>"><i class="fa fa-sign-out"></i></a>
        </li>
      </ul>
    </nav>
<?php
